...
export function InputBar({
  ...
  const fallbackLabel = (selectedModelPath?.trim()) || '未设置模型'
    ...
    models && models.length > 0 ? (
      <div className="relative" ref={dropdownRef}>
        <button
          ...
          className="flex max-w-[180px] items-center gap-1 rounded px-1.5 py-0.5 text-[11px] text-[#6b6b6b] transition-colors hover:bg-[#3c3c3c] hover:text-[#aaaaaa]"
        >
      ...
      <span
        className="max-w-[180px] truncate px-1.5 py-0.5 text-[11px] text-[#6b6b6b]"
        title={selectedModelPath?.trim() || undefined}
      >
        {fallbackLabel}
      </span>
    )
